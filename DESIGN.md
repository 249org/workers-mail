...
8px grid. App chrome `px-8` on desktop, `px-4` plus safe-area insets on a phone. Mail is three hairline-split panes on a wide screen; the page never scrolls — panes do. Below 768px it is one pane at a time: the list is the home screen, a tap opens the reader full width, and folders slide in from the left. Compose lives on the list toolbar. Each pane starts with a 56px toolbar so search and reading actions sit on one axis. The folder rail collapses to a 56px icon strip (`[`) on desktop — that control lives on the list, to the left of search; Compose fills the sidebar toolbar. The message list can hide so the reader fills the remaining width (`]`, Enter) — that control lives on the reader, and on a phone it is Back. Only the 56px reader toolbar sticks; the letterhead (Newsreader subject, byline, date) scrolls away. The letter itself is a centered 42rem sheet, not edge-to-edge UI text. Settings is an index and a spread: a paper rail of destinations with a live one-line status, and a full-height card page for the open topic. On a phone the index becomes a horizontal strip. Moving between destinations is instant — the rail and spread swap on click, with no transition, and visited pages stay mounted. The spread head holds the Newsreader title; the body is a ruled ledger or form that fills the pane. Stats sit in a ruled strip of cells (borders on children, no outer double line). Empty states are centered, with a highlight-subtle icon well.
